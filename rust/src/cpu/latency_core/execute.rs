use super::{LatencyCore, Mode};
use crate::cpu::{exception::Exception, instr::*, register::RegisterFile};

impl LatencyCore {
    pub fn execute(&mut self, instr: u64) -> Result<(Option<u64>, Option<u64>), Exception> {
    ...
    fn execute_load(
        ...
        rs1: u64,
        instr: u64,
    ) -> Result<(Option<u64>, Option<u64>), Exception> {
        ...
        let addr = self.xreg_read(rs1).wrapping_add(imm);

        match funct3 {
            0b000 => {
                // lb
                let value = self.read(addr, 8)?;
                self.xreg_write(rd, value as i8 as i64 as u64);
            }
            0b001 => {
                // lh
                let value = self.read(addr, 16)?;
                self.xreg_write(rd, value as i16 as i64 as u64);
            }
            0b010 => {
                // lw
                let value = self.read(addr, 32)?;
                self.xreg_write(rd, value as i32 as i64 as u64);
            }
            0b011 => {
                // ld
                let value = self.read(addr, 64)?;
                self.xreg_write(rd, value as i64 as u64);
            }
            0b100 => {
                // lbu
                let value = self.read(addr, 8)?;
                self.xreg_write(rd, value);
            }
            0b101 => {
                // lhu
                let value = self.read(addr, 16)?;
    ...
    fn execute_store(
        ...
        let rs1 = self.xreg_read(rs1);
        let addr = rs1.wrapping_add(imm);

        let value = self.xreg_read(rs2);
    ...
    fn execute_op_imm(
        ...
        let a = self.xreg_read(rs1);
        let funct6 = funct7 >> 1;
        match funct3 {
            0b000 => {
                // addi
                self.xreg_write(rd, a.wrapping_add(imm12))
            }
            0b001 => {
                // slli
                let shamt = (instr >> 20) & 0x3f;
                self.xreg_write(rd, a << shamt);
            }
            0b010 => {
                //slti
                self.xreg
            ...
            0b011 => {
                //sltiu
                self.xreg_write(rd, if a < imm12 { 1 } else { 0 })
            }
            0b100 => {
                //xori
                self.xreg_write(rd, a ^ imm12);
            }
            0b101 => {
                let shamt = (instr >> 20) & 0x3f;
                ...
                match funct6 {
                    ...
                    0b01_0000 => {
                        // srai
                        self.xreg_write(rd, ((a as i64) >> shamt) as u64)
                    }
                    _ => return Err(Exception::IllegalInstruction(instr)),
                }
            }
    ...
    fn execute_op_imm32(
        ...
        match funct3 {
            ...
            0b001 => {
                // slliw
                self.xreg_write(rd, (a << shamt) as i32 as i64 as u64);
            }
            0b101 => {
                match funct6 {
                    0b00_0000 => {
                    ...
                    0b01_0000 => {
                        // sraiw
                        self.xreg_write(rd, ((a as i32) >> shamt) as i64 as u64)
                    }
                    _ => return Err(Exception::IllegalInstruction(instr)),
                }
            }
    ...
    fn execute_op(
        ...
        let a = self.xreg_read(rs1);
        let b = self.xreg_read(rs2);
        match (funct3, funct7) {
            (0x0, 0x00) => {
                //add
                self.xreg_write(rd, a.wrapping_add(b))
            }
            (0x0, 0x01) => {
                //mul
                self.xreg
            ...
            (0x0, 0x20) => {
                //sub
                self.xreg_write(rd, a.wrapping_sub(b))
            }
            (0x1, 0x00) => {
                // sll
                let shamt = b & 0x3f;
            ...
            (0x3, 0x00) => {
                // sltu
                self.xreg_write(rd, if a < b { 1 } else { 0 });
            }
            (0x3, 0x01) => {
                // mulhu
                // unsigned × unsigned
    ...
    fn execute_op32(
        ...
        match (funct3, funct7) {
            (0x0, 0x00) => {
                //addw
                self.xreg_write(rd, a.wrapping_add(b) as i32 as i64 as u64)
            }
            (0x0, 0x01) => {
                //mulw
                self.xreg
            ...
            (0x0, 0x20) => {
                //subw
                self.xreg_write(rd, a.wrapping_sub(b) as i32 as u64)
            }
            (0x1, 0x00) => {
                // sllw
                self.xreg_write(rd, (a << shamt) as i32 as i64 as u64);
    ...
    fn execute_jal(
        ...
        self.xreg_write(rd, self.pc.wrapping_add(8));
    ...
    fn execute_jalr(
        ...
        let target = ((self.xreg_read(rs1) as i64).wrapping_add(imm12)) & !1;

        self.next_pc = (target as u64).wrapping_sub(4);
        self.xreg_write(rd, next);
    ...
    fn execute_branch(
        ...
        let a = self.xreg_read(rs1);
        let b = self.xreg_read(rs2);
        let new_pc = self.pc.wrapping_add(imm);
    ...
    #[inline]
    fn execute_misc_mem(&mut self, instr: u64) -> Result<(Option<u64>, Option<u64>), Exception> {
        self.stats.instr.instr_fence += 1;
        // self.("fence_instructions");

        let funct3 = (instr & 0x00007000) >> 12;
        match funct3 {
            0x0 | 0x1 => Ok((None, None)),
            _ => Err(Exception::IllegalInstruction(instr)),
        }
    }
    ...
    fn execute_auipc(
        ...
        self.stats.instr.instr_arith += 1;
        self.xreg_write(rd, self.pc.wrapping_add(imm20));
        Ok((None, None))
    }
